	...
	AmsifySuggestags.prototype = {
		...
		*/
		...
		setSuggestionsEvents : function() {
			...
			$(".amsify-suggestags-lista").find(this.classes.listItem).click(function(){
				_self.addTag($(this).data('val'));
				//$(this).hide();
				_self.updateSuggestionList();
				//location.reload(); 
				//_self.amsifySuggestags()
				_self.setSuggestionsEvents();
				$(_self.selectors.sTagsInput).val('').focus();
		...
		createList : function() {
			...
			var items = $(this.selector).val().split(',');
			var nlinst = this.settings.suggestions;
			
			var itemss = $(this.selector).val();
			//alert($(this.selector).val());
			//alert(this.settings.suggestions);
			var cclist="";
			var dfg = new Array();
		for(var i=0; i<nlinst.length; i++){
			var sulist=nlinst[i];
			//alert(sulist);
			var a = items.indexOf(sulist);
			//alert(sulist+"@"+a);
			if(a=="-1"){
				//alert(sulist);
				cclist+=sulist;
			...
			var listHTML  = '';
			var listHTMLs  = '';
			//alert(this.settings.tagvLimit);
			var limt=this.settings.tagvLimit;
			$.each(dfg, function(index, item){
				...
				if(index>=limt){
					listHTML += '<li class="'+_self.classes.listItem.substring(1)+'" data-val="'+value+'" style="display: none;">'+tag+'</li>';
					listHTMLs += '<a href="javascript:void(0);" class="'+_self.classes.listItem.substring(1)+'" data-val="'+value+'" style="display: none;">'+tag+'</a>';
				
				} else {
					 listHTML += '<li class="'+_self.classes.listItem.substring(1)+'" data-val="'+value+'">'+tag+'</li>';
					 listHTMLs += '<span class="coma-sep">,</span> <a href="javascript:void(0);" class="'+_self.classes.listItem.substring(1)+'" data-val="'+value+'">'+tag+'</a>';
				}
			...
			if(_self.settings.noSuggestionMsg) {
				...
				listHTMLs += '<a href="javascript:void(0);" class="'+_self.classes.noSuggestion.substring(1)+'">'+_self.settings.noSuggestionMsg+'</a>';
			}
			$(".vlist").html(listHTMLs.substring(32));
		...
		setIcon : function() {
			...
			if(this.settings.type == 'bootstrap') {
				return '<span class="skills-close '+removeClass+'">&times;</span>';
